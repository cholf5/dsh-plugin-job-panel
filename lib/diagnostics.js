...
export function createDiagnostics({ logger } = {}) {
	...
	return {
		/**
		 * Merge one patch into the record and persist it.
		 * @param {object} patch - shallow merge into the current record.
		 * @returns {Promise<void>} resolved once this patch is persisted (or
		 *   logged as failed) — awaiting it makes writes observable for tests
		 *   and boot determinism; callers that do not await keep the old
