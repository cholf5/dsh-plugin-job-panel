...
/**
 * Normalize an optional `sessionId` field: absent, null, and empty all mean
 * "not supplied" (an empty string arrives when a caller has no session —
 * e.g. a panel mounted outside a live session — and must not be a 400).
...
function normalizeOptionalSessionId(value) {
	return isValidString(value, MAX_SESSION_ID_LENGTH) ? value : undefined;
}
 ...
 * @param {{ tap: import("./tap.js").JobTap, jobs: object | undefined, logger: (message: string) => void, onQuery?: (record: { route: string, jobId: string | undefined, tapped: boolean, hasStreams: boolean }) => void }} deps
 *   onQuery: optional per-request observer (diagnostics), fired after each
 *   dispatch with the answer the route is about to give.
 ...
 */
export function registerRoutes({ tap, jobs, logger, onQuery }) {
	const report = (route, jobId, tapped, hasStreams) => {
		try {
			onQuery?.({ route, jobId, tapped, hasStreams });
		} catch {
	...
	const outputHandler = async (request) => {
		...
		if (jobIdError !== undefined) {
			report("output", jobId, false, false);
			return jsonResponse({ error: jobIdError }, 400);
		}
		...
		for (const name of ["stdoutOffset", "stderrOffset"]) {
			...
			if (!isSafeOffset(value)) {
				report("output", jobId, false, false);
				return jsonResponse({ error: `${name} must be a non-negative safe integer` }, 400);
			}
			offsets[name] = value;
		}
	...
	const stopHandler = async (request) => {
		...
		if (jobIdError !== undefined) {
			report("stop", body?.jobId, false, false);
			return jsonResponse({ error: jobIdError }, 400);
		}
		const sessionId = normalizeOptionalSessionId(body?.sessionId);
		...
		const callerId = resolveCallerId(tap, body.jobId, sessionId);
	...
	// `requestBody` is semantically required: when it is absent the bridge's
	// requestBodyMode returns undefined, the bridge takes the streaming branch,
	// and constructing a GET Request with a body throws — the webserver's
	// catch-all turns that throw into the 400 the panel used to see.
	return [
		{ path: OUTPUT_PATH, methods: ["GET", "HEAD"], requestBody: "buffered", fetch: outputHandler },
		{ path: FULL_PATH, methods: ["GET", "HEAD"], requestBody: "buffered", fetch: fullHandler },
