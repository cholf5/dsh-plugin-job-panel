...
export async function apply(ctx) {
	...
	diagnostics.update({ event: "apply-entered", services });

	try {
		const tap = new JobTap({
			onTrack: () => diagnostics.update({ lastJobTrackedAt: new Date().toISOString() })
		});
		...
		diagnostics.update({ event: "routes-registered", routes: paths });
	} catch (error) {
